...
// All indices are long long to prevent overflow when N=32M and width grows large.
// Without this, tid*2*width overflows int32 in the final passes.
__global__ void gpu_merge_kernel(int *src, int *dst, long long n, long long width)
{
    ...
    // No right partition - just copy so dst stays consistent
    if (mid >= right) {
...
// Returns pointer to whichever buffer holds the sorted result after ping-pong
int *gpu_mergeSort(int *d_data, int *d_temp, int n)
{
...
int main()
{
    ...
    int *d_result = gpu_mergeSort(d_data, d_temp, N);
